...
import kotlinx.android.synthetic.main.item_pk_detail.view.*
 ...
 * Description:
 */
class PkDetailRvAp(val items: List<String>, val context: Context, var itemclick: ItemClick, var itemInnrclick: ItemInnerClick) : RecyclerView.Adapter<PkDetailRvAp.MainRvApViewHolder>() {
    ...
    class MainRvApViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        fun bindData(data: String, click: OnClick, innrclick: OnInnerClick) {
            with(data) {
                itemView.tv_item_content.text = data
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MainRvApViewHolder {
        return MainRvApViewHolder(LayoutInflater.from(context).inflate(R.layout.item_pk_detail, parent, false))
    }
