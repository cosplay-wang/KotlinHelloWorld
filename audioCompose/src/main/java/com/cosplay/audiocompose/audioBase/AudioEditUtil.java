...
import com.cosplay.audiocompose.util.ByteUtil;
...
public class AudioEditUtil {
   public static Audio insertOutAudio  = new Audio();
    //wave头文件大小
   ...
   public static void pcm2Wav(String pcm,String wav){
     ...
     */
    public static void insertAudioWithSame(Audio srcAudio, AudioFragment coverAudio) {
        ...
        try {
            ...
            //writeWavHeader(newFos);
            final int srcStartPos = getPositionFromWave(srcStartTime, sampleRate, channels, bitNum);
            final int srcEndPos = getPositionFromWave(srcEndTime, sampleRate, channels, bitNum);
            ...
            int remainSize = (int) (srcFileSize - srcEndPos);
            if (remainSize > 0) {

              //  srcFis.seek(WAVE_HEAD_SIZE + srcEndPos);
                copyData(srcFis, newFos, remainSize,0);
        ...
        //new File(srcWavePath).delete();
        // 转换临时文件为源文件
        AudioEncodeUtil.convertPcm2Wav(tempOutPcmPath, insertOutAudio.getPath(), sampleRate, channels, bitNum);
        //删除临时文件
     ...
     * @param coverAudio   覆盖音频路径
     */
    public static void replaceAudioWithSame(Audio srcAudio, AudioFragment coverAudio) {
        float srcStartTime = coverAudio.mPosition;
        float srcEndTime = coverAudio.mDuration;
        String srcWavePath = srcAudio.getPath();
        String coverWavePath = coverAudio.mFile;
     ...
     * @param cooySize 复制大小
     */
    public static void copyData(RandomAccessFile fis, RandomAccessFile fos, final int cooySize) {
     ...
     * @param cooySize 复制大小
     */
    private static void copyData(RandomAccessFile fis, RandomAccessFile fos, final int cooySize,int startposition) {
     ...
     */
    public static void copyData(RandomAccessFile fis, RandomAccessFile fos, final int cooySize, final float volumeValue) {
    ...
    public static final int FREQUENCY = 16000; // sample rate
    public static final int BITS = 16;
    public static final int BYTE_PER_MS = CHANNELS * FREQUENCY * BITS / 1000 / 8;
    public static final byte EMPTY_BYTE = 0;

    private static final int CHUNK_SIZE = 2304;
    public static void writeWavHeader(RandomAccessFile file) {
        try {
            /* RIFF header */
            ...
            file.writeInt(Integer.reverseBytes(CHANNELS * FREQUENCY * BITS / 8)); // BPSecond
